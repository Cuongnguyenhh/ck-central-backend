...
using CK.Central.Core.POS.HealthCheck.DbContexts;
...
using CK.Central.Core.POS.HealthCheck.Repositories;
...
using CK.Central.Core.POS.HealthCheck.Extensions;

namespace CK.Central.API.POS.HealthCheck
{
    public static class InitialRegistration
    {
        public static IServiceCollection AddAuth(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "CK.Central.API.POS.HealthCheck", Version = "v1" });
        ...
        public static IServiceCollection AddDependencyInjection(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddScoped<IElasticsearchPOSHealthCheckService, ElasticsearchPOSHealthCheckService>();
            services.AddScoped<IRedisCachePOSHealthCheckService, RedisCachePOSHealthCheckService>();
            services.AddScoped<IKafkaProducerPOSHealthCheckService, KafkaProducerPOSHealthCheckService>();
        ...
        public static IServiceCollection AddHealthCheck(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            //services.AddHealthChecks()
            //    .AddDbContextCheck<MasterDbContext>("DbContextCheck");

            //services.AddHealthChecks()
            //    .AddDbContextCheck<SlaveDbContext>("DbContextCheck");
            ...
            //services
            //    .AddHealthChecks()
            //    .AddNpgSql(new ConnectionString(configuration.GetConnectionString("DefaultConnection")).ToString(), string.Empty, null, "MasterPgsqlDbHealthCheck");

            //services.AddHealthChecks()
            //    .AddRedis(new ConnectionString(configuration.GetConnectionString("DefaultConnection")).ToString(), "RedisServerHealthCheck");

            //services.AddHealthChecks()
            //    .AddKafka(null, "RedisServerHealthCheck");

            //services.AddHealthChecks()
            //    .AddElasticsearch(new ConnectionString(configuration.GetConnectionString("DefaultConnection")).ToString(), "ElasticsearchHealthCheck");

            //services.AddHealthChecks()
            //    .AddRabbitMQ(new ConnectionString(configuration.GetConnectionString("DefaultConnection")).ToString(), null, "RabbitMQHealthCheck");
        ...
        (context, loggerConfiguration) =>
        {
            ...
            {
                loggerConfiguration.MinimumLevel.Override("CK-Central-POS-HealthCheck", LogEventLevel.Debug);
            }
            ...
            if (!string.IsNullOrEmpty(elasticUrl))
            {
                loggerConfiguration.WriteTo.Elasticsearch(
                    new ElasticsearchSinkOptions(new Uri(elasticUrl))
                    {
                        ...
                        InlineFields = true,
                        IndexFormat = "CK-Central-POS-HealthCheck-Logs-{0:yyyy.MM.dd}",
                        NumberOfReplicas = 2,
                        NumberOfShards = 2
                    });
            }
