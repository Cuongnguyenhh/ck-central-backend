...
namespace CK.Central.API.POS.GrabMart.Order.Controllers
{
    ...
    public class OrderCancelController : ControllerEntityBase<OrderCancelEntity>
    {
        ...
        public OrderCancelController(ILogger<OrderCancelController> logger,
            IMediator mediator,
            IOrderCancelRepository repository,
        ...
        public async Task<ActionResult<CancelOrderResponseDto>> CancelOrder([FromBody] CancelOrderCommand command)
        {
            try
            {
                var result = await _mediator.Send(command);
                _logger.LogInformation(JsonConvert.SerializeObject(result));
                await _kafkaProducerService.ProduceEvent(Core.Domain.GrabMart.Constants.KafkaTopicReceive.request_cancel_order_detail, JsonConvert.SerializeObject(result));
                return Ok(result);
            }
            catch (Exception ex) 
            {
