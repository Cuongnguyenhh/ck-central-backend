...
namespace CK.Central.API.POS.GrabMart.Order.Controllers
{
    ...
    public class OrderSubmitController : ControllerEntityBase<OrderSubmitEntity>
    {
        private readonly IMediator _mediator;
        private readonly IOrderSubmitRepository _repository;
        private readonly ILogger<OrderSubmitController> _logger;
        ...
        public OrderSubmitController(ILogger<OrderSubmitController> logger,
            IMediator mediator,
            IOrderSubmitRepository repository,
        ...
        public async Task<ActionResult<CreateOrderResponseDto>> CreateOrder([FromBody] CreateOrderCommand command)
        {
            try
            {
                var result = await _mediator.Send(command);
                _logger.LogInformation(JsonConvert.SerializeObject(result));
                await _kafkaProducerService.ProduceEvent(Core.Domain.GrabMart.Constants.KafkaTopicSend.pos_response_create_order_detail, JsonConvert.SerializeObject(result));
                return Ok(result);
            }
            catch (Exception ex)
            {
