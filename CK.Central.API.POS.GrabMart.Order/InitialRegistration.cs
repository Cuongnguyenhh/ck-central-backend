...
using CK.Central.Core.POS.GrabMart.Order.Repositories;
using Microsoft.AspNetCore.RateLimiting;

namespace CK.Central.API.POS.GrabMart.Order
{
    public static class InitialRegistration
    {
        public static IServiceCollection AddAuth(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "CK.Central.API.POS.GrabMart.Order", Version = "v1" });
        ...
        public static IServiceCollection AddDependencyInjection(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddScoped<IElasticsearchPOSGrabMartOrderService, ElasticsearchPOSGrabMartOrderService>();
            services.AddScoped<IRedisCachePOSGrabMartOrderService, RedisCachePOSGrabMartOrderService>();
            services.AddScoped<IKafkaProducerPOSGrabMartOrderService, KafkaProducerPOSGrabMartOrderService>();
            services.AddScoped<IAppSettingService, AppSettingService>();
            services.AddScoped<IJWTokenService, JWTokenService>();
            services.AddScoped<IMediator, Mediator>();

            services.AddScoped<IOrderSubmitRepository, OrderSubmitRepository>();
        ...
        (context, loggerConfiguration) =>
        {
            ...
            {
                loggerConfiguration.MinimumLevel.Override("CK-Central-POS-GrabMart-Order", LogEventLevel.Debug);
            }
            ...
            if (!string.IsNullOrEmpty(elasticUrl))
            {
                loggerConfiguration.WriteTo.Elasticsearch(
                    new ElasticsearchSinkOptions(new Uri(elasticUrl))
                    {
                        ...
                        InlineFields = true,
                        IndexFormat = "CK-Central-POS-GrabMart-Order-Logs-{0:yyyy.MM.dd}",
                        NumberOfReplicas = 2,
                        NumberOfShards = 2
                    });
            }
