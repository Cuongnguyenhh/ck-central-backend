...
using CK.Central.Core.POS.DbContexts;
using CK.Central.Core.Abstract.Repositories;
using CK.Central.Core.POS.Abstract.Services;
using CK.Central.Core.POS.Abstract.Repositories;
using CK.Central.Core.POS.Services;
using CK.Central.Core.POS.Repositories;
...
namespace CK.Central.API.POS
{
    public static class InitialRegistration
    {
        public static IServiceCollection AddAuth(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "CK.Central.API.POS", Version = "v1" });
        ...
        public static IServiceCollection AddDependencyInjection(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddScoped<IElasticsearchPOSService, ElasticsearchPOSService>();
            services.AddScoped<IRedisCachePOSService, RedisCachePOSService>();
            services.AddScoped<IKafkaConsumerPOSService, KafkaConsumerPOSService>();
            services.AddScoped<IKafkaProducerPOSService, KafkaProducerPOSService>();
        ...
        (context, loggerConfiguration) =>
        {
            ...
            {
                loggerConfiguration.MinimumLevel.Override("CK-Central-POS", LogEventLevel.Debug);
            }
            ...
            if (!string.IsNullOrEmpty(elasticUrl))
            {
                loggerConfiguration.WriteTo.Elasticsearch(
                    new ElasticsearchSinkOptions(new Uri(elasticUrl))
                    {
                        ...
                        InlineFields = true,
                        IndexFormat = "CK-Central-POS-Logs-{0:yyyy.MM.dd}",
                        NumberOfReplicas = 2,
                        NumberOfShards = 2
                    });
            }
