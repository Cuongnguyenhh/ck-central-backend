...
using CK.Central.Core.CMS.Generate.Services;
using Microsoft.AspNetCore.RateLimiting;
using CK.Central.Core.CMS.Generate.Abstract.Repositories;
using CK.Central.Core.CMS.Generate.Repositories;

namespace CK.Central.API.CMS.Generate
{
    public static class InitialRegistration
    {
        public static IServiceCollection AddAuth(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "CK.Central.API.CMS.Generate", Version = "v1" });
        ...
        public static IServiceCollection AddDependencyInjection(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddScoped<IElasticsearchCMSGenerateService, ElasticsearchCMSGenerateService>();
            services.AddScoped<IMemoryCacheCMSGenerateService, MemoryCacheCMSGenerateService>();
            services.AddScoped<IRedisCacheCMSGenerateService, RedisCacheCMSGenerateService>();
            services.AddScoped<IRabbitMQProducerCMSGenerateService, RabbitMQProducerCMSGenerateService>();
            services.AddScoped<IAppSettingService, AppSettingService>();
            services.AddScoped<IJWTokenService, JWTokenService>();

            services.AddScoped<IGenerationActivityRepository, GenerationActivityRepository>();
            services.AddScoped<IGenerationHistoricalRepository, GenerationHistoricalRepository>();
            services.AddScoped<IGenerationPipelineRepository, GenerationPipelineRepository>();
        ...
        (context, loggerConfiguration) =>
        {
            ...
            {
                loggerConfiguration.MinimumLevel.Override("CK-Central-CMS-Generate", LogEventLevel.Debug);
            }
            ...
            if (!string.IsNullOrEmpty(elasticUrl))
            {
                loggerConfiguration.WriteTo.Elasticsearch(
                    new ElasticsearchSinkOptions(new Uri(elasticUrl))
                    {
                        ...
                        InlineFields = true,
                        IndexFormat = "CK-Central-CMS-Generate-Logs-{0:yyyy.MM.dd}",
                        NumberOfReplicas = 2,
                        NumberOfShards = 2
                    });
            }
