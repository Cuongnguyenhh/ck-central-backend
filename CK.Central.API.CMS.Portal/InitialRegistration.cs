...
using CK.Central.Core.CMS.Portal.Repositories;
using MongoDB.Driver;
using CK.Central.Core.DbContexts;
using CK.Central.Core.CMS.Portal.Abstract.Services;
using CK.Central.Core.CMS.Portal.Services;

namespace CK.Central.API.CMS.Portal
{
    public static class InitialRegistration
    {
        public static IServiceCollection AddAuth(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "CK.Central.API.CMS.Portal", Version = "v1" });
        ...
        {
            services.AddScoped<DbContextBase, MasterDbContext>();
            services.AddScoped<IMasterDbContext, MasterDbContext>();
            services.AddScoped<DbContextBase, SlaveDbContext>();
            ...
            services.AddScoped<IElasticsearchCMSPortalService, ElasticsearchCMSPortalService>();
            services.AddScoped<IMemoryCacheCMSPortalService, MemoryCacheCMSPortalService>();
            services.AddScoped<IRedisCacheCMSPortalService, RedisCacheCMSPortalService>();
            services.AddScoped<IRabbitMQProducerCMSPortalService, RabbitMQProducerCMSPortalService>();
            services.AddScoped<IAppSettingService, AppSettingService>();
            services.AddScoped<IJWTokenService, JWTokenService>();
            services.AddScoped<IRabbitMQBaseService, RabbitMQBaseService>();

            services.AddScoped<ICampaignRepository, CampaignRepository>();
            services.AddScoped<ICampaignItemRepository, CampaignItemRepository>();
            services.AddScoped<ICampaignServiceRepository, CampaignServiceRepository>();
            services.AddScoped<ICategoryRepository, CategoryRepository>();
            services.AddScoped<ICustomerRepository, CustomerRepository>();
            services.AddScoped<IDepartmentRepository, DepartmentRepository>();
            services.AddScoped<IDisclaimerRepository, DisclaimerRepository>();
            services.AddScoped<IItemDetailRepository, ItemDetailRepository>();
            services.AddScoped<IItemDisclaimerRepository, ItemDisclaimerRepository>();
        ...
        (context, loggerConfiguration) =>
        {
            ...
            {
                loggerConfiguration.MinimumLevel.Override("CK-Central-CMS-Portal", LogEventLevel.Debug);
            }
            ...
            if (!string.IsNullOrEmpty(elasticUrl))
            {
                loggerConfiguration.WriteTo.Elasticsearch(
                    new ElasticsearchSinkOptions(new Uri(elasticUrl))
                    {
                        ...
                        InlineFields = true,
                        IndexFormat = "CK-Central-CMS-Portal-Logs-{0:yyyy.MM.dd}",
                        NumberOfReplicas = 2,
                        NumberOfShards = 2
                    });
            }
