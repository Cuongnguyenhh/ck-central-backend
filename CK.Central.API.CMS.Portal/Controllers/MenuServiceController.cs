...
namespace CK.Central.API.CMS.Portal.Controllers
{
    ...
    public class MenuServiceController : ControllerEntityBase<MenuServiceEntity>
    {
        ...
        public MenuServiceController(ILogger<MenuServiceController> logger, IMenuServiceRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveByMenuIdAsync(Guid menuId)
        {
            try
            {
                Expression<Func<MenuServiceEntity, bool>> filter = x => x.MenuUUID == menuId && x.IsActive == true && x.IsDeleted == false;
