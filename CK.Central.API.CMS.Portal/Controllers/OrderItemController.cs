...
namespace CK.Central.API.CMS.Portal.Controllers
{
    ...
    public class OrderItemController : ControllerEntityBase<OrderItemEntity>
    {
        ...
        public OrderItemController(ILogger<OrderItemController> logger, IOrderItemRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveByOrderIdAsync(Guid orderId)
        {
            try
            {
                Expression<Func<OrderItemEntity, bool>> filter = x => x.OrderUUID == orderId && x.IsActive == true && x.IsDeleted == false;
