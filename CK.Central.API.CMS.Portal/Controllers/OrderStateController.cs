...
namespace CK.Central.API.CMS.Portal.Controllers
{
    ...
    public class OrderStateController : ControllerEntityBase<OrderStateEntity>
    {
        ...
        public OrderStateController(ILogger<OrderStateController> logger, IOrderStateRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveByOrderIdAsync(Guid orderId)
        {
            try
            {
                Expression<Func<OrderStateEntity, bool>> filter = x => x.OrderUUID == orderId && x.IsActive == true && x.IsDeleted == false;
