...
namespace CK.Central.API.CMS.Portal.Controllers
{
    ...
    public class MenuTemplateController : ControllerEntityBase<MenuTemplateEntity>
    {
        private readonly IMenuTemplateRepository _repository;
        ...
        public MenuTemplateController(ILogger<MenuTemplateController> logger, IMenuTemplateRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveByMenuIdAsync(Guid menuId)
        {
            try
            {
                Expression<Func<MenuTemplateEntity, bool>> filter = x => x.MenuUUID == menuId && x.IsActive == true && x.IsDeleted == false;
