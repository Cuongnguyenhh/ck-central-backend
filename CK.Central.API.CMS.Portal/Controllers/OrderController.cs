...
namespace CK.Central.API.CMS.Portal.Controllers
{
    ...
    public class OrderController : ControllerEntityBase<OrderEntity>
    {
        ...
        [HttpGet]
        [Route("get-all-orders-active")]
        public async Task<IActionResult> GetAllActiveOrderAsync()
        {
            return Ok(await _repository.GetAllOrdersActiveAsync());
        }

        [HttpGet]
        [Route("get-order-by-id")]
        public async Task<IActionResult> GetOrderByIdAsync(Guid pkId)
        {
            //Guid Id = new Guid(pkId);
            return Ok(await _repository.GetOrderByIdAsync(pkId));
        }

        [HttpGet]
        [Route("get-order-active-by-id")]
        public async Task<IActionResult> GetOrderActiveByIdAsync(string? pkId)
        {
            Guid Id = new Guid(pkId);
            return Ok(await _repository.GetOrderActiveByIdAsync(Id));
        }

        [HttpPost]
        [Route("delete-order-active-by-id")]
        public async Task<IActionResult> DeleteOrderByIdAsync([FromBody] Guid Id)
        {
            return Ok(await _repository.DeleteOrderByIdAsync(Id));
        }
    }
}