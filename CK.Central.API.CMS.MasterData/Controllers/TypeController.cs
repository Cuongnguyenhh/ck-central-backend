...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class TypeController : ControllerEntityBase<TypeEntity>
    {
        ...
        public TypeController(ILogger<TypeController> logger, ITypeRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<TypeEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<TypeEntity>>(RedisCachingKey.TypeListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<TypeEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<TypeEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<TypeEntity>>(RedisCachingKey.TypeListActive, cacheData));
