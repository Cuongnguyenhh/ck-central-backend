...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class DefinitionController : ControllerEntityBase<DefinitionEntity>
    {
        ...
        public DefinitionController(ILogger<DefinitionController> logger, IDefinitionRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<DefinitionEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<DefinitionEntity>>(RedisCachingKey.DefinitionListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<DefinitionEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<DefinitionEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<DefinitionEntity>>(RedisCachingKey.DefinitionListActive, cacheData));
