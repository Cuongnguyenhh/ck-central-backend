...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class ProvinceController : ControllerEntityBase<ProvinceEntity>
    {
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<ProvinceEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<ProvinceEntity>>(RedisCachingKey.ProvinceListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<ProvinceEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<ProvinceEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<ProvinceEntity>>(RedisCachingKey.ProvinceListActive, cacheData));
