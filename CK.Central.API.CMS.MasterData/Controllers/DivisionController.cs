...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class DivisionController : ControllerEntityBase<DivisionEntity>
    {
        ...
        public DivisionController(ILogger<DivisionController> logger, IDivisionRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<DivisionEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<DivisionEntity>>(RedisCachingKey.DivisionListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<DivisionEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<DivisionEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<DivisionEntity>>(RedisCachingKey.DivisionListActive, cacheData));
