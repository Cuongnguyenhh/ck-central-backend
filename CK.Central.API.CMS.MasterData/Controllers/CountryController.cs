...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class CountryController : ControllerEntityBase<CountryEntity>
    {
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<CountryEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<CountryEntity>>(RedisCachingKey.CountryListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<CountryEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<CountryEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<CountryEntity>>(RedisCachingKey.CountryListActive, cacheData));
