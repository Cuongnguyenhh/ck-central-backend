...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class UnitController : ControllerEntityBase<UnitEntity>
    {
        ...
        public UnitController(ILogger<UnitController> logger, IUnitRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<UnitEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<UnitEntity>>(RedisCachingKey.UnitListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<UnitEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<UnitEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<UnitEntity>>(RedisCachingKey.UnitListActive, cacheData));
