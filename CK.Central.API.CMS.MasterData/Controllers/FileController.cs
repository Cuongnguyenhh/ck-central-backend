...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class FileController : ControllerEntityBase<FileEntity>
    {
        ...
        public FileController(ILogger<FileController> logger, IFileRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<FileEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<FileEntity>>(RedisCachingKey.FileListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<FileEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<FileEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<FileEntity>>(RedisCachingKey.FileListActive, cacheData));
