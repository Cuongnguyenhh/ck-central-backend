...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class StatusController : ControllerEntityBase<StatusEntity>
    {
        ...
        public StatusController(ILogger<StatusController> logger, IStatusRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<StatusEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<StatusEntity>>(RedisCachingKey.StatusListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<StatusEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<StatusEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<StatusEntity>>(RedisCachingKey.StatusListActive, cacheData));
