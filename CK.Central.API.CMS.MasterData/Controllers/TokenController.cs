...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class TokenController : ControllerEntityBase<TokenEntity>
    {
        ...
        public TokenController(ILogger<TokenController> logger, ITokenRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<TokenEntity>>(
                ...
                Expression<Func<TokenEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<TokenEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<TokenEntity>>(RedisCachingKey.TokenListActive, cacheData));
