...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class MessageController : ControllerEntityBase<MessageEntity>
    {
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<MessageEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<MessageEntity>>(RedisCachingKey.MessageListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<MessageEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<MessageEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<MessageEntity>>(RedisCachingKey.MessageListActive, cacheData));
