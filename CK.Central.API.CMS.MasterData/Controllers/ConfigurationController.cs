...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class ConfigurationController : ControllerEntityBase<ConfigurationEntity>
    {
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<ConfigurationEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<ConfigurationEntity>>(RedisCachingKey.ConfigurationListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<ConfigurationEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<ConfigurationEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<ConfigurationEntity>>(RedisCachingKey.ConfigurationListActive, cacheData));
