...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class EmailController : ControllerEntityBase<EmailEntity>
    {
        ...
        public EmailController(ILogger<EmailController> logger, IEmailRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<EmailEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<EmailEntity>>(RedisCachingKey.EmailListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<EmailEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<EmailEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<EmailEntity>>(RedisCachingKey.EmailListActive, cacheData));
