...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class ServiceController : ControllerEntityBase<ServiceEntity>
    {
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<ServiceEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<ServiceEntity>>(RedisCachingKey.ServiceListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<ServiceEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<ServiceEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<ServiceEntity>>(RedisCachingKey.ServiceListActive, cacheData));
