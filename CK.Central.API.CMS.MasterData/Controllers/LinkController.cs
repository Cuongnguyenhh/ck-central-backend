...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class LinkController : ControllerEntityBase<LinkEntity>
    {
        ...
        public LinkController(ILogger<LinkController> logger, ILinkRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<LinkEntity>>(
                ...
                Expression<Func<LinkEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<LinkEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<LinkEntity>>(RedisCachingKey.LinkListActive, cacheData));
