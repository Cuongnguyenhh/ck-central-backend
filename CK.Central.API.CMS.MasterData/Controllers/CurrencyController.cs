...
namespace CK.Central.API.CMS.MasterData.Controllers
{
    ...
    public class CurrencyController : ControllerEntityBase<CurrencyEntity>
    {
        ...
        public CurrencyController(ILogger<CurrencyController> logger, ICurrencyRepository repository,
        ...
        public async Task<IActionResult> GetAllActiveAsync()
        {
            try
            {
                var cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<CurrencyEntity>>(
                    async () => await _redisCacheService.GetData<IEnumerable<CurrencyEntity>>(RedisCachingKey.CurrencyListActive));

                if (cacheData != null) return Ok(cacheData);

                Expression<Func<CurrencyEntity, bool>> query = x => x.IsActive == true && x.IsDeleted == false;

                cacheData = await _retryPolicy.ExecuteAsync<IEnumerable<CurrencyEntity>>(
                    async () => await _repository.Filter(query));

                if (cacheData == null) return Ok(null);

                await _retryPolicy.ExecuteAsync<bool>(
                    async () => await _redisCacheService.SetData<IEnumerable<CurrencyEntity>>(RedisCachingKey.CurrencyListActive, cacheData));
