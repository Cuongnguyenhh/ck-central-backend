...
using CK.Central.Core.GrabMart.Job.DbContexts;
using CK.Central.Core.Abstract.Repositories;
using CK.Central.Core.GrabMart.Job.Abstract.Services;
using CK.Central.Core.GrabMart.Job.Abstract.Repositories;
using CK.Central.Core.GrabMart.Job.Services;
using CK.Central.Core.GrabMart.Job.Repositories;
...
namespace CK.Central.API.GrabMart.Job
{
    public static class InitialRegistration
    {
        public static IServiceCollection AddAuth(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "CK.Central.API.GrabMart.Job", Version = "v1" });
        ...
        public static IServiceCollection AddDependencyInjection(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddScoped<IElasticsearchGrabMartJobService, ElasticsearchGrabMartJobService>();
            services.AddScoped<IRedisCacheGrabMartJobService, RedisCacheGrabMartJobService>();
            services.AddScoped<IKafkaProducerGrabMartJobService, KafkaProducerGrabMartJobService>();
            services.AddScoped<IAppSettingService, AppSettingService>();
            services.AddScoped<IJWTokenService, JWTokenService>();
            services.AddScoped<IMediator, Mediator>();

            services.AddScoped<IJobHistoricalRepository, JobHistoricalRepository>();
            ...
            services.AddScoped<ITaskHandledRepository, TaskHandledRepository>();
            services.AddScoped<ITaskHistoricalRepository, TaskHistoricalRepository>();
            services.AddScoped<ITaskPipelineRepository, TaskPipelineRepository>();
        ...
        public static IServiceCollection AddJobHangfires(this IServiceCollection services, IConfiguration configuration)
        {
            services.AddEntityFrameworkNpgsql().AddDbContext<MasterDbContext>(options => {
                options.UseNpgsql(configuration.GetConnectionString("MasterPgsqlDbConnection"));
            });

            services.AddHangfire(x => x.UsePostgreSqlStorage(configuration.GetConnectionString("MasterPgsqlDbConnection")));
        ...
        (context, loggerConfiguration) =>
        {
            ...
            {
                loggerConfiguration.MinimumLevel.Override("CK-Central-GrabMart-Job", LogEventLevel.Debug);
            }
            ...
            if (!string.IsNullOrEmpty(elasticUrl))
            {
                loggerConfiguration.WriteTo.Elasticsearch(
                    new ElasticsearchSinkOptions(new Uri(elasticUrl))
                    {
                        ...
                        InlineFields = true,
                        IndexFormat = "CK-Central-GrabMart-Job-Logs-{0:yyyy.MM.dd}",
                        NumberOfReplicas = 2,
                        NumberOfShards = 2
                    });
            }
