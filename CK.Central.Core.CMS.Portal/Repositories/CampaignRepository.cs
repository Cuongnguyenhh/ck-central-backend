...
namespace CK.Central.Core.CMS.Portal.Repositories
{
    public class CampaignRepository : BaseRepository<CampaignEntity>, ICampaignRepository
    {
        ...
        public CampaignRepository(IUnitOfWork context, MasterDbContext masterContext, SlaveDbContext slaveDbContext,
        ...
        public async Task<CampaignEntity> InsertMongoDBCampaignAsync(CampaignEntity entity)
        {
            ...
            var db = new MongoDBContext(new DbContextOptionsBuilder<MongoDBContext>().UseMongoDB(_mongoClient, _mongoDatabase.DatabaseNamespace.DatabaseName).Options);

            IMongoCollection<CampaignEntity> campaignCollection = _mongoDatabase.GetCollection<CampaignEntity>("");

            await campaignCollection.Find(_ => true).ToListAsync();

            await campaignCollection.InsertOneAsync(entity);

            await db.Campaigns.AddAsync(entity);
